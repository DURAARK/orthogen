...
#include <vector>
  ...
  template <class T>
    ...
    public:
        ...
        inline const T *data() const { return &pdata[1]; }
        inline const std::vector<T>& getData() const { return pdata; }
		inline int buffersize() const { return W*H*BPP / sizeof(T); }
        ...
        { 
          return pdata[(y * W + x) * BPP / sizeof(T) + ch]; 
        }
        ...
        {
            memset(&pdata[0], value, W*H*BPP/sizeof(T));
        }
    };
